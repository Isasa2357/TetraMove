...
from Field.Exception import GameOverException, type_GameOver
...
class TetrisField:
    def __init__(self, height:int, width:int, firstMinoID:str=None, side_margin: int=2, bottom_margin=2, position: list=None, first_downCounter_threshold: int=20):
        ...
        self.__minoQue = Queue()
        self.init_minoQue()
        self.__holdMino:Holdmino = None
        self.place_nextMino(firstMinoID, position=position)

        self.__downCounter = 0
        self.__downCounter_threshold = first_downCounter_threshold
        self.__min_downCounter_threshold = 5
    ...
    def init_minoQue(self):
        count = 5 - self.__minoQue.qsize()
        for _ in range(count):
            self.__minoQue.put(gen_minoCalculator())

    def place_nextMino(self, mino:str=None, position: list=None):
        next_minoCalculator = self.__minoQue.get()
        self.__minoQue.put(gen_minoCalculator())
        self.__holdMino = Holdmino(self.__field, minoType=next_minoCalculator.ID(), position=position)

        # ミノが配置可能かを判定
        if not self.can_write_mino(self.__holdMino.get_minoGrid(), self.__holdMino.get_position()):
            raise GameOverException(self.__field, type_GameOver['cant_placeMino_firstPos'])
        
        # TODO: 初期位置に設置できない可能性を考慮してない

    def levelup(self, step=1):
        '''
        downCounter_thresholdを減らし，ミノの落下速度を早めることで難易度を上げる
        '''
        self.__downCounter_threshold = max(self.__downCounter_threshold - 1, self.__min_downCounter_threshold)
    ...
    def judge_gameover(self):
        ...
        ゲームオーバー判定
        '''
        gameover_areas_pos = self.get_gameover_pos()

        # print(self.__field)
        for idx in gameover_areas_pos:
            y, x = idx
            if self.__field[y][x] not in writable_block_state:
                return True
        return False
    ...
    def get_minoQue(self) -> list[MinoCalculator]:
        with self.__minoQue.mutex:
            return copy.deepcopy(list(self.__minoQue.queue))
    
    def get_downCounter_threshold(self):
        return self.__downCounter_threshold
...
class TestField:
    ...
    class GameOverException:
        def __init__(self, field: np.array, type_GO: int):
            ...
            Args:
                field: ゲームオーバー時のフィールドの状態
                type_GO: type_GameOVerのこと．ゲームオーバーした原因を示すint型の値
                    1: ミノの初期配置位置に既にミノが配置されている
            '''
            self.__field = field
            self.__